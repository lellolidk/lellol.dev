...
async function initializeVisitorCounter() {
        try {
            const API_NINJAS_KEY = 'AV1M0Cd4boeeT4JHrnFSug==CGEYMYARhN4I2Sm9';
            const COUNTER_ID = 'kanake_global_views';
            const url = `https://api.api-ninjas.com/v1/counter?id=${encodeURIComponent(COUNTER_ID)}&hit=true`;
            const res = await fetch(url, { headers: { 'X-Api-Key': API_NINJAS_KEY }, cache: 'no-store' });
